...
- MoE 구조에서는 Self-Attention을 공유하며, 여러 개의 MLP 중 일부만 활성화
- 라우터(Router) 모델이 N개의 MLP 중 일부를 선택해 처리
...
- 또한, 과적합이 발생할 수 있어 일반적인 데이터는 일반적인 답변을 할 수 있도록 복습
...
- Instruct model: 질의응답/ 지시사항 형식의 템플릿 이해
  - 단순 Completion 에서, 질의응답과 지시사항의 데이터로 파인 튜닝한 모델
    ```
    <|start_header_id|>assistant<|end_header_id|> 이후부터 생성하도록 훈련
    ```
  - Instruct model 에 파인튜닝하는걸 Instruct tuning(IT) 라 한다.
  - 문제 범위가 좁은 경우 it 만 수행한다.
...
- 좋은 RLHF 를 만들기는 좋은 보상모델이 필요한데, 이게 쉽지 않음
- Rejection sampling + SFT 로 대체
  - Instruct Model 에서 샘플링으로 다양한 데이터를 생성하고, 고품질의 답변으로 추가 SFT 수행하는 방법
- 답변 품질이 떨어질 때 rl 을 쓸수잇다. 보상함수는 어려우니 dpo, rs + sft 를 고려한다.
